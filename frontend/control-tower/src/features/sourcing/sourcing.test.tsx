...
const AWARD = awardPayload.parse({
  ...
  comparison: {
    ...
    quotes: [
      ...
      { partner_id: 9, partner_name: "Hidráulica Alterna SAC", po_name: "P00901", source: "reply", currency: "USD", total: 1176, lead_days: 18, score: 70, rank: 2, reasons: ["9.8% above the lowest", "fastest: 18 day(s)"], lines: [] },
  ...
  it("shows the comparison, marks the recommendation and lets the buyer pick another winner", async () => {
    ...
    render(
      <I18nProvider initial="en">
        <AwardCard payload={AWARD} chosen={8} onChoose={onChoose} canAct />
      </I18nProvider>,
    );
    expect(screen.getByText("Round #1")).toBeInTheDocument();
    expect(screen.getByText("2 of 3 invited suppliers replied")).toBeInTheDocument();
    expect(screen.getByText("[CBEA-LHN] Válvula × 10")).toBeInTheDocument();
