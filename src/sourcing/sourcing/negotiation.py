...
def plan_offer(
    *,
    ...
    if target_override is not None:
        candidates.append((target_override, "the buyer's target"))
    if last_paid is not None and last_paid > 0:
        candidates.append((last_paid, f"our last paid price of {last_paid:.2f}"))
    positive = [p for p in competing if p > 0]
    if positive:
        candidates.append((min(positive), f"a competing price of {min(positive):.2f}"))
    if candidates:
        ...
        target, basis = round(current_price * (1.0 - cap_pct / 200.0), 2), "a standard opening ask"
