"""ASGI entrypoint for the sourcing agent.

Routes: the shared system surface, ``POST /approvals/callback``, the rounds
read model behind the bearer token, and the A2A agent.
"""
...
def build_app() -> FastAPI:
    application = create_application(
        settings,
        version=__version__,
        routers=[approvals.router, rounds.router],
        modules=module_list(),
        startup=[_startup],
        shutdown=[_shutdown],
    )
    if settings.a2a_token:
        mount(
            application,
            SourcingHandler(application.state.injector.get(AgentProvider)),
            agent_spec(settings.sourcing.public_url),
