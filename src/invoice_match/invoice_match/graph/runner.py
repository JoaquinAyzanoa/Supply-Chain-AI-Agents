"""Run and resume the graph; turn its state into an ``InvoiceMatchResult``."""
...
from invoice_match.graph.nodes.decide import BILL_STEP
...
class InvoiceMatchAgent:
    ...
    async def run(self, task: InvoiceMatchTask) -> InvoiceMatchResult:
        run_id = new_id("run")
    ...
    async def resume(self, case_id: str, decision: dict[str, Any]) -> InvoiceMatchResult:
    ...
    async def _finish(self, state: dict[str, Any], budget: RunBudget) -> InvoiceMatchResult:
    ...
    def result_from(self, state: dict[str, Any]) -> InvoiceMatchResult:
        return result_from(state, language=self._language)


def result_from(state: dict[str, Any], *, language: Language = "en") -> InvoiceMatchResult:
    task = InvoiceMatchTask.model_validate(state["task"])
    if state.get("outcome"):
        outcome = Outcome.model_validate(state["outcome"])
    else:
        pending = pending_for(state, BILL_STEP) or _any_pending(state)
        outcome = Outcome(
    ...
    created = state.get("created_bill") or {}
    return InvoiceMatchResult(
        ...
        invoice=_model(InvoiceData, state.get("invoice")),
        match=_model(BillMatch, state.get("match")),
        bill_id=created.get("id"),
        bill_name=created.get("name"),
...
__all__ = ["AGENT_NAME", "InvoiceMatchAgent", "result_from"]
