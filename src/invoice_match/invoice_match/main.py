"""ASGI entrypoint for the invoice matching agent.
...
settings = Settings(service_name="invoice_match")
...
def build_app() -> FastAPI:
    ...
    if settings.a2a_token:
        mount(
            application,
            InvoiceMatchHandler(application.state.injector.get(AgentProvider)),
            agent_spec(settings.invoice_match.public_url),
