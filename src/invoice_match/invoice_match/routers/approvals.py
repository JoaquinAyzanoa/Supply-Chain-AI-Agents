"""``POST /approvals/callback``: Odoo (or the Control Tower) resolved a ``vendor_bill`` approval.

Same contract as the other agents' callbacks: signed body, ``thread_id`` is
...
from invoice_match.service import AgentProvider
from sc_core.a2a.events import EventPublisher, SignedBody
from sc_core.schema.a2a import InvoiceMatchResult
...
def run_finished(result: InvoiceMatchResult, approval_id: int) -> AgentRunFinished:
