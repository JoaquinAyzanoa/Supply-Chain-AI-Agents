"""Run and resume the graph; turn its state into a ``SupplierPerformanceResult``."""
...
from supplier_performance.nodes.steps import SCORE_STEP
from supplier_performance.ports import PerformancePorts


class SupplierPerformanceAgent:
    def __init__(
        self,
    ...
    async def run(self, task: SupplierPerformanceTask) -> SupplierPerformanceResult:
        run_id = new_id("run")
    ...
    async def resume(self, case_id: str, decision: dict[str, Any]) -> SupplierPerformanceResult:
    ...
    async def _finish(self, state: dict[str, Any], budget: RunBudget) -> SupplierPerformanceResult:
    ...
    def result_from(self, state: dict[str, Any]) -> SupplierPerformanceResult:
        return result_from(state, language=self._language)


def result_from(state: dict[str, Any], *, language: Language = "en") -> SupplierPerformanceResult:
    task = SupplierPerformanceTask.model_validate(state["task"])
    if state.get("outcome"):
        outcome = Outcome.model_validate(state["outcome"])
    else:
        pending = pending_for(state, SCORE_STEP) or _any_pending(state)
        outcome = Outcome(
    ...
    return SupplierPerformanceResult(
        ...
        applied=PerformanceApplied.model_validate(applied) if applied else None,
...
__all__ = ["AGENT_NAME", "SupplierPerformanceAgent", "result_from"]
