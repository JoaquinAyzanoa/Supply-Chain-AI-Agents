"""ASGI entrypoint for the supplier performance agent.

Routes: the shared system surface, ``POST /approvals/callback``, the ranking
and scores behind the bearer token, and the A2A agent.
"""
...
def build_app() -> FastAPI:
    application = create_application(
        settings,
        version=__version__,
        routers=[approvals.router, ranking.router],
        modules=module_list(),
        startup=[_startup],
        shutdown=[_shutdown],
    )
    if settings.a2a_token:
        mount(
            application,
            SupplierPerformanceHandler(application.state.injector.get(AgentProvider)),
            agent_spec(settings.supplier_performance.public_url),
