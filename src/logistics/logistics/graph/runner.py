"""Run and resume the graph; turn its state into a ``LogisticsResult``."""
...
from logistics.graph.nodes.eta import ETA_STEP
from logistics.graph.nodes.mail import SEND_STEP
from logistics.infra.ports import LogisticsPorts
...
class LogisticsAgent:
    ...
    async def run(self, task: LogisticsTask) -> LogisticsResult:
        run_id = new_id("run")
    ...
    async def resume(self, case_id: str, decision: dict[str, Any]) -> LogisticsResult:
    ...
    async def _finish(self, state: dict[str, Any], budget: RunBudget) -> LogisticsResult:
    ...
    def result_from(self, state: dict[str, Any]) -> LogisticsResult:
        return result_from(state, language=self._language)


def result_from(state: dict[str, Any], *, language: Language = "en") -> LogisticsResult:
    task = LogisticsTask.model_validate(state["task"])
    if state.get("outcome"):
        outcome = Outcome.model_validate(state["outcome"])
    else:
        pending = (
            pending_for(state, ETA_STEP) or pending_for(state, SEND_STEP) or _any_pending(state)
        )
        outcome = Outcome(
    ...
    return LogisticsResult(
        ...
        outcome=outcome,
        po_name=task.po_name or ctx.get("name"),
        shipment=_model(ShipmentInfo, state.get("shipment")),
        proposal=_model(ChangeProposal, state.get("proposal")),
        reconciliation=_model(ReceiptReconciliation, state.get("reconciliation")),
...
__all__ = ["AGENT_NAME", "LogisticsAgent", "result_from"]
